      ...
      m_lagValue(0.f), m_maxValue(100.f), m_isCritical(false), m_isOnCooldown(false), m_pulseTime(0.f)
{
    ...
    m_bgRect.setFillColor(sf::Color(15, 15, 22, 200)); // pó³przezroczysty ciemnoszary/niebieski
    ...
    // Wyrównanie etykiety pionowo do œrodka po lewej stronie
...
void UIBar::update(float dt, float currentValue, float maxValue, bool isOnCooldown) {
    ...
    // Ograniczenie bie¿¹cej wartoœci do przedzia³u [0, max]
    if (m_currentValue < 0.f) m_currentValue = 0.f;
    if (m_currentValue > m_maxValue) m_currentValue = m_maxValue;

    // Inicjalizacja wartoœci opóŸnienia w pierwszej klatce lub przy utracie synchronizacji
    if (m_lagValue < 0.01f && m_currentValue > 0.01f) {
        m_lagValue = m_currentValue;
    }

    // Powolna interpolacja wartoœci opóŸnienia w dó³.
    // Jeœli bie¿¹ca wartoœæ jest wiêksza ni¿ opóŸnienie (np. leczenie lub ³adowanie), opóŸnienie dogania j¹ natychmiast
    if (m_currentValue >= m_lagValue) {
        m_lagValue = m_currentValue;
    } else {
        // Powolne przejœcie opóŸnienia
        m_lagValue -= (m_lagValue - m_currentValue) * dt * 2.0f;
        if (m_lagValue < m_currentValue) m_lagValue = m_currentValue;
    }

    // Sprawdzenie krytycznego poziomu zdrowia (poni¿ej 25%)
    m_isCritical = (m_labelText.getString() == "HP" && (m_currentValue / m_maxValue) < 0.25f);

    m_pulseTime += dt;

    // Aktualizacja koloru ramki w zale¿noœci od stanów
    if (m_isCritical) {
        // Czerwona migaj¹ca ramka przy niskim poziomie zdrowia
        ...
        // Niebieska migaj¹ca ramka podczas czasu odnowienia (cooldown)
    ...
    // Aktualizacja tekstu wartoœci
    std::stringstream ss;
    if (m_labelText.getString() == "NITRO") {
        // Wyœwietlanie jako procent
    ...
    // Wyrównanie tekstu wartoœci do prawej strony
...
void UIBar::draw(sf::RenderWindow& window) {
    ...
    // Rysowanie paska opóŸnienia (jeœli jest wiêkszy ni¿ bie¿¹cy pasek)
    if (m_lagRect.getSize().x > m_fillRect.getSize().x) {
        window.draw(m_lagRect);
    }

    // Rysowanie w³aœciwego paska
    if (m_fillRect.getSize().x > 0.f) {
        window.draw(m_fillRect);
        window.draw(m_sheenRect); // rysowanie b³yszcz¹cego efektu szk³a na wierzchu
    }
...
UIUpgradeMenu::UIUpgradeMenu(sf::Vector2f resolution, const sf::Font& font)
    : m_font(font)
{
    // Ustawienie wymiarów i pozycji (wyœrodkowany panel 640x480)
    ...
    // Obramowanie z neonowym b³êkitem
    m_borderRect.setPosition(m_position);
    m_borderRect.setSize(m_size);
    m_borderRect.setFillColor(sf::Color::Transparent);
...
UIFractionsMenu::UIFractionsMenu()
    : m_position(0.f, 0.f), m_size(0.f, 0.f) {}
...
void UIFractionsMenu::update(const std::vector<Fraction>& fractions) {
    for (size_t i = 0; i < 4 && i < fractions.size(); ++i) {
        ...
        if (trust >= 70) {
            ...
            m_fractionsText[i].setFillColor(sf::Color(230, 230, 230)); // Bialy - neutralny
        } else {
